...
original long term projection to cefi format
    ...
    for forcing in forcing_dirs:
        for file in glob.glob(f'{ori_path}/{forcing}/*.nc'):
            ...
            if filename not in portal_data.StaticFile.filenames:
                ...
                file_type_list = filename.split('.')
                file_format_list = file_type_list[0].split('_')
                ...
                new_dir = os.path.join(cefi_portal_base,cefi_rel_path,forcing)
                ...
                filename = portal_data.MultiDecadeFilename(
                    variable=variable,
                    region=region_file,
                ...
                file_global_attrs = portal_data.GlobalAttrs(
                    cefi_rel_path = os.path.join(cefi_rel_path,forcing),
                    cefi_filename = filename,
                    cefi_variable = variable,
                    cefi_ori_filename = file.split('/')[-1],
                    cefi_ori_category = file.split('/')[-1].split('.')[0].split('_')[0],
